...
export default async function Home() {
  ...
  const [liveAds, { data: venueRows }] = await Promise.all([
    getLiveAds(),
    admin
      .from('venues')
      .select('id, name')
          ...
          <div className="mx-auto grid w-full max-w-6xl items-center gap-10 px-6 py-12 lg:grid-cols-[1.05fr_1fr] lg:gap-14 lg:py-20">
            <div className="text-center lg:text-left">
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-1.5 text-xs text-muted-foreground lg:justify-start">
                {['From $50 / screen / month', 'Month to month', 'We can design your ad'].map(
                  (t) => (
          ...
          <Card>
            <CardContent className="flex flex-col items-center gap-4 p-8 text-center">
              ...
              <p className="max-w-md text-sm text-muted-foreground">
                Pricing scales with each location&apos;s traffic, starting at $50/screen. Pay only
                for the screens you pick, no minimum. Cancel anytime, and we can design your ad if
                you need it. No long-term contracts, and you always see the price before you buy.
              </p>
              <Link href="/signup" className={cn(buttonVariants({ size: 'lg' }))}>
                Start advertising
              </Link>
