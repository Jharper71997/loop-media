...
import { Eye, EyeOff, MonitorX, Plus, Power, RefreshCw, RotateCcw, Save, X } from 'lucide-react'
...
// Take a house slide off THIS screen, or put it back. Sits next to the seconds
// field: one is "how long", this is "at all". The screen picks it up on its next
// ~30s poll, so this is reversible in under a minute and needs no confirm.
export function HouseEnabledToggle({
  tvId,
  slideKind,
  enabled,
  label,
  ...
  return (
    <Button
      ...
      className="shrink-0"
      disabled={pending}
      aria-label={`${enabled ? 'Take off' : 'Put back on'} this screen: ${label}`}
      onClick={() =>
        start(async () => {
          const res = await updateHouseSlideEnabled(tvId, slideKind, !enabled)
          if (res.error) toast.error(res.error)
          else {
            toast.success(enabled ? 'Off — gone from this screen.' : 'Back on this screen.')
            router.refresh()
          }
        })
      }
    >
      ...
      <span className="ml-2">{enabled ? 'Take off' : 'Put back'}</span>
    </Button>
  )
}
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <Button
          ...
          variant="outline"
          disabled={pending}
          onClick={() => send('sleep', 'Sleep queued — the screen takes it on its next sync.')}
        >
        ...
        <Button
          size="sm"
          disabled={pending}
          onClick={() => send('wake', 'Wake queued — the screen takes it within a minute.')}
        >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">
              {hoursLabel
                ? `Venue hours: ${hoursLabel}. The screen wakes itself a few minutes before open and sleeps at close, on its own alarm — so it keeps the schedule even if the internet is down.`
                : 'Set this venue’s open hours first, or the screen has no schedule to keep.'}
            </p>
          </div>
          <Button
            size="sm"
            ...
            onClick={() =>
              start(async () => {
                ...
                if (res.error) toast.error(res.error)
                  ...
                  toast.success(
                    sleepWhenClosed
                      ? 'Back to 24/7 — a wake is on its way to the screen.'
