...
// comes back around every few minutes anyway — a question a patron half-read is
// still on the board when it returns.
const TRIVIA_SLIDE_SECONDS = 15
...
function Player({
  ...
  const applyShellState = useCallback(
    async (data: Manifest) => {
      ...
      // panel is off — at that point this page is paused and cannot be asked.
      try {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        // On screen: fast, so the close-out and the next question land on time.
...
function TriviaSlide({
  ...
  // interval, so the close-out and the next question aren't late. Once per round: an
